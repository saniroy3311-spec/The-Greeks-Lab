...
"""
    ...
    # 7. Replace active price alerts header to remove duplicate inputs
    ...
    # 8. Inject core javascript variables after activeCandleOpenTime
    js_vars = """
...
"""
    ...
    # 9. Replace getActiveIndicators definition
    old_get_active_indicators = """  function getActiveIndicators(togglesContainerId) {
    ...
    new_get_active_indicators = """  function getActiveIndicators(togglesContainerId) {
    if (Array.isArray(togglesContainerId)) return togglesContainerId;
    const container = $(togglesContainerId);
    if (!container) return [];
    const checkedInputs = container.querySelectorAll('input[type="checkbox"]:checked');
    return Array.from(checkedInputs).map(input => input.getAttribute('data-indicator'));
  }"""
    html = html.replace(old_get_active_indicators, new_get_active_indicators)

    # 10. Replace updateChartIndicators references
    ...
    # 11. Replace liveChartEngine initialization block (exact replacement)
    old_live_init = """  liveChartEngine = createChartEngine(container, {
    theme: 'dark',
    pricePrecision: 2,
    chartType: 'candles',
    ...
    new_live_init = """  liveChartEngine = createChartEngine(container, {
    ...
    html = html.replace(old_live_init, new_live_init)

    # 12. Replace scalperChartEngine initialization block (exact replacement)
    old_scalper_init = """  scalperChartEngine = createChartEngine(container, {
    theme: 'light',
    pricePrecision: 2,
    chartType: 'candles',
    onActiveToolChange: (tool) => {
    ...
    new_scalper_init = """  scalperChartEngine = createChartEngine(container, {
    theme: document.body.classList.contains('light-theme') ? 'light' : 'dark',
    pricePrecision: 2,
    chartType: 'candles',
    onActiveToolChange: (tool) => {
    ...
    html = html.replace(old_scalper_init, new_scalper_init)

    # 13. Inject settings loading into engine initializations
    ...
    # 14. Replace checkAlerts tune & duration parameters
    ...
    # 15. Replace loadDrawings definition to add history stack & settings
  ...
  loadChartSettings(engine);
    ...
    # 16. Replace change event listener for symbol and timeframe (removing activeCandleOpenTime = null)
    old_change_listener = """  ['symbol','tf'].forEach(id => $(id).addEventListener('change', () => {
    ...
    new_change_listener = """  ['symbol','tf'].forEach(id => $(id).addEventListener('change', () => {
    wsLastClosedTime = null;
    if (activeTab === 'live-signal') {
      load(true);
    } else {
    ...
    html = html.replace(old_change_listener, new_change_listener)

    # 17. Inject the core functions and startup boots
    core_functions = """
// -------------------------------------------------------------
...
// Continuous 60fps loop for shaded risk-reward boxes to eliminate lagging
...
"""
  ...
  if (savedLiveInds) {
    let parsed = JSON.parse(savedLiveInds);
    if (parsed.includes('ema') && !parsed.includes('ema9')) {
      parsed = parsed.filter(x => x !== 'ema');
      parsed.push('ema9', 'ema15');
    }
    liveActiveIndicators = parsed;
  }
  const savedScalperInds = localStorage.getItem('greeks_active_indicators_scalper');
  if (savedScalperInds) {
    let parsed = JSON.parse(savedScalperInds);
    if (parsed.includes('ema') && !parsed.includes('ema9')) {
      parsed = parsed.filter(x => x !== 'ema');
      parsed.push('ema9', 'ema15');
    }
    scalperActiveIndicators = parsed;
  }

  startShadedBoxesLoop();
  loadAlerts();"""
